...
from moxie_messages import chat_memory_request
    ...
    if payload.get("event_id") == request_id:
        ...
        if payload["result"] == 9:
            ...
            if payload["result"] != 0:
                ...
                outp = json.dumps(payload, indent=2)
                log_and_print(f"Memory Response: {outp}")
...
def make_session():
    ...
    if args.nolog:
        ...
        session_root = os.path.join(".", "log", f"Memory_{session_uuid}")
...
def prompt_and_send(input_auid = None):
    global request_id
    ...
    if input_auid:
        auid_to_check = input_auid
    else:
        auid = input(f"Enter AUID [{session_auid}]: ")
        auid_to_check = auid if auid else session_auid
    log_only(f"AUID: {auid_to_check}")
    request_id = str(uuid.uuid4())
    # need a unique session ID to get clean memories
    query_session_id = str(uuid.uuid4())
    mmsg = chat_memory_request(query_session_id, auid_to_check, request_id, topic=topic_from_args())
...
parser.add_argument("--in_file", dest="in_file", metavar="in_file", type=str, action="store", help="CSV Input file with AUIDs")
...
if True:
    ...
    if args.in_file:
        with open(args.in_file, "r") as f:
            for line in f:
                prompt_and_send(input_auid=line[:-1])
    else:
        while True:
            prompt_and_send()
else:
